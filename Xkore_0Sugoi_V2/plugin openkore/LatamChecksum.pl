...
sub checkServer {
	...
	if ( grep { $master->{serverType} eq $_ } qw(ROla) ) {
		...
		$ip_socket   = $config{ip_socket}   || $ENV{CHECKSUM_BRIDGE_HOST} || '127.0.0.1';
		$port_socket = $config{port_socket} || $ENV{CHECKSUM_BRIDGE_PORT} || 2349;
		warning "LatamChecksum: usando bridge TCP $ip_socket:$port_socket\n";
	}
}
...
sub calc_checksum {
	...
	my $socket = IO::Socket::INET->new(
		PeerHost => $ip_socket,
		PeerPort => $port_socket,
		Proto    => 'tcp',
		Timeout  => $TIMEOUT
	);

	unless ($socket) {
		error "LatamChecksum: Failed to connect to checksum bridge at $ip_socket:$port_socket!\n";
		return 0; # fallback checksum
	}
	binmode($socket);
	...
	# Protocolo sem username: payload + counter + seed_high + seed_low
	my $packet = $data . pack("NNN", $counter, $seed_high, $seed_low);
	
	unless (print $socket $packet) {
		error "LatamChecksum: Failed to send data to bridge - $!\n";
		$socket->close();
		return 0;
	}
	
	# Read checksum response: 1B checksum + seed_high + seed_low + counter
	my $rest;
	my $r = sysread($socket, $rest, 1 + 4 + 4 + 4); # checksum + seed_high + seed_low + counter
	$socket->close();
	unless (defined $r && $r == 13) {
		error "LatamChecksum: Failed to read checksum payload from bridge\n";
		return 0;
	}

	my ($checksum, $resp_seed_high, $resp_seed_low, $server_counter) = unpack("C N N N", $rest);
