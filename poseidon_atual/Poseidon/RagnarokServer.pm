...
sub getFreeClients {
	...
	for (my $i = 0; $i < @{$clients}; $i++) {
		if ($clients->[$i]) {
			if ($clients->[$i]{connectedToMap} && !$clients->[$i]{boundUsername}) {
				push(@free, $clients->[$i]);
			}
		}
	}
...
sub query
{
	my ($self, $packet) = @_;
	my $clients = $self->clients();
	print "Request from ".$packet->{username}." \n" if $packet->{username};
	
	# search for our bound client
	for (my $i = 0; $i < @{$clients}; $i++) {
		if ($clients->[$i]) {
			if ($clients->[$i]{connectedToMap}) {
				if ($clients->[$i]{boundUsername} && $clients->[$i]{boundUsername} eq $packet->{username}) {
					print "Bound to ".$packet->{username}." \n";
					$clients->[$i]{boundUsername} = $packet->{username};
					$clients->[$i]->send($packet->{packet});
					$clients->[$i]{query_state} = 'requesting';
					print sprintf("Requesting from client %s \n", $i);
					$clients->[$i]{last_request} = time;
					return $i;
				}
			}
		}
	...
	# not found... try to free one of them
	for (my $i = 0; $i < @{$clients}; $i++) {
		if ($clients->[$i]) {
			if ($clients->[$i]{connectedToMap}) {
				if ($clients->[$i]{last_request} && (time > ($clients->[$i]{last_request} + ($config{max_client_inactivity}*60)))) {
					print sprintf("Freeing client %s after inactivity\n", $i);
					$clients->[$i]{last_request} = undef;
					$clients->[$i]{boundUsername} = undef;
				}
			}
		}
	}
...
sub readResponse {
	my ($self) = @_;
	return $self->_consumeResponse($self, 1);
}

sub readClientResponse {
	my ($self, $client) = @_;
	return $self->_consumeResponse($client, 0);
}

sub _consumeResponse {
	my ($self, $source, $is_server) = @_;
	return unless $source && defined $source->{response};

	my $resp = $source->{response};
	chop($resp) if ($self->{type}->{$config{server_type}}->{checksum} == 1);
	$source->{response} = undef;

	if ($is_server) {
		$self->{state} = 'ready';
	} else {
		$source->{query_state} = 'ready';
	}
...
sub ParsePacket {
	...
	#	send characters_info
		...
		if ($length > 0) {
			$client->{response} = pack("v", $packet_id) . substr($msg,2,$length);
		} else {
			$client->{response} = pack("v", $packet_id);
		};
		$client->{query_state} = 'replied';
